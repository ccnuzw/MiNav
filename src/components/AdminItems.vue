<template>
  <div class="bg-white dark:bg-dark-card p-6 rounded-lg shadow">
    <div class="flex justify-between items-center mb-6">
        <h2 class="text-xl font-bold dark:text-white">Items Management</h2>
            ...
            <tbody>
                <tr v-for="item in items" :key="item.id" class="border-b dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-800">
                    ...
                    <td class="px-6 py-4 space-x-2">
                        <button @click="editItem(item)" class="text-indigo-600 hover:text-indigo-900 dark:text-indigo-400 dark:hover:text-indigo-300">Edit</button>
                        <button @click="deleteItem(item.id)" class="text-red-600 hover:text-red-900 dark:text-red-400 dark:hover:text-red-300">Delete</button>
                    </td>
                </tr>
            </tbody>
        </table>
    ...
    <div v-if="showAddModal || editingItem" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
        <div class="bg-white dark:bg-dark-card p-6 rounded-lg w-full max-w-md">
            <h3 class="text-lg font-bold mb-4 dark:text-white">{{ editingItem ? 'Edit Item' : 'Add Item' }}</h3>
            <form @submit.prevent="handleSubmit" class="space-y-4">
                ...
                <div>
                    <label class="block text-sm font-medium dark:text-gray-300">URL</label>
                ...
                <div>
                    <label class="block text-sm font-medium dark:text-gray-300">Category</label>
                ...
                <div>
                     <label class="block text-sm font-medium dark:text-gray-300">Icon (FontAwesome Class)</label>
                ...
                <div class="flex justify-end space-x-2 mt-4">
                    <button type="button" @click="closeModal" class="px-4 py-2 text-gray-500 hover:text-gray-700 dark:text-gray-400">Cancel</button>
